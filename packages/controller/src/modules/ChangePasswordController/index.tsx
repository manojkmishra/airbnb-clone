...
{ submit = async (values: ForgotPasswordChangeMutationVariables) => 
  { console.log('/controller/changepassctrl-values=',values);
    const {  data: { forgotPasswordChange }    } = await this.props.mutate({  variables: values  });
    console.log('/controller/changepassctrl-forgotpasschange=',forgotPasswordChange);
    if (forgotPasswordChange) {  return normalizeErrors(forgotPasswordChange);    }
    return null;
  };
